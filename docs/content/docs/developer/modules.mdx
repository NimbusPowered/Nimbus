...
For module-fired events (`NimbusEvent.ModuleEvent("my-module", "SOMETHING",
data)`), register a formatter so console output looks native:

```kotlin title="Event formatter"
...
1. Build your module JAR:
   ...
   <Callout type="warn">
   If you use `com.github.johnrengelman.shadow`, **exclude** core runtime
   dependencies so your fat JAR doesn't bundle Ktor, Exposed, kotlinx, or
   SLF4J — Nimbus core already provides them. Bundling duplicates causes
   `LinkageError` at module load.
   ```kotlin title="build.gradle.kts (shadowJar)"
   tasks.shadowJar {
       dependencies {
   ...
   ```
   Only shadow **your own** third-party transitive dependencies
   (domain-specific libraries Nimbus doesn't ship).
   </Callout>
