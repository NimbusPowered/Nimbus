...
The plugin entry point is `NimbusPermsPlugin` (`dev.nimbuspowered.nimbus.perms.NimbusPermsPlugin`), a standard `JavaPlugin` that reads its API connection from JVM system properties (`nimbus.api.url`, `nimbus.api.token`). It requires the [Nimbus SDK](/docs/developer/sdk) and only activates on Nimbus-managed services.
...
All providers implement `dev.nimbuspowered.nimbus.perms.provider.PermissionProvider`:
...
`PermissibleInjector` (`dev.nimbuspowered.nimbus.perms.handler.PermissibleInjector`) extends `PermissibleBase` to add wildcard matching that Bukkit does not support natively:
...
The plugin is auto-deployed to every backend service at runtime by the
Permissions module (installed by default) via
`ServiceFactory.resolveModulePlugins()`. Deployment can be disabled with
`[permissions] deploy_plugin = false` in `nimbus.toml`. Templates stay
user-owned; the JAR is never written into `templates/global/plugins/`.
