...
├── database/      → DatabaseManager, MigrationManager, Exposed ORM tables, MetricsCollector, AuditCollector
...
├── service/       → Service lifecycle, process management, StateSyncManager
├── setup/         → First-run wizard
├── stress/        → StressTestManager (simulated player load testing)
├── system/        → System-level utilities (memory reading, process inspection)
...
├── StateSyncClient       → Pull-on-start / push-on-stop delta sync with controller canonical store
├── TemplateSync          → Downloads and caches templates from controller
└── TlsHelper.kt          → Keystore loading, SHA-256 fingerprint verification
...
nimbus-module-players (Player Module)
├── PlayerManager, PlayerSession, PlayerHistory
├── PlayerRoutes, PlayerCommand
└── PlayerTables (Exposed DB tables)
