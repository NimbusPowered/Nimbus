...
1. Every **heartbeat interval** (default: 10 seconds), ping all `READY` services via [Server List Ping](https://wiki.vg/Server_List_Ping) to update player counts
...
```java title="Game Plugin — Custom State"
// In your game plugin:
...
```toml title="config/groups/<group>.toml"
[group.scaling]
...
```toml title="config/groups/<group>.toml — Event Mode"
[group.scaling]
min_instances = 5
max_instances = 20
...
```toml title="config/modules/scaling/<GroupName>.toml"
[schedule]
enabled = true
...
```bash title="Nimbus — Smart Scaling"
...
```text title="REST API — Smart Scaling"
