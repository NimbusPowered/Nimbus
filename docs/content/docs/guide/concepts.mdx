...
See [group config reference](/docs/config/groups#groupsync) for the full option list.
...
1. **`templates/global/`** — Copied first. Contains files shared across all backend servers (Paper, Purpur, Leaf, etc.). The nimbus-sdk plugin is auto-deployed here.
...
| **Leaf** | Minecraft server (Paper fork, performance + stability) | [Leaf API](https://api.leafmc.one) |
