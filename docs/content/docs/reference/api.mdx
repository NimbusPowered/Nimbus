...
Admin-only endpoints (require the admin token): `/api/reload`, `/api/stress/*`, `/api/files/*`, `/api/config`, `/api/loadbalancer`, `/api/nodes`, `/api/doctor`.
...
Provided by the **Backup** module (migration range 7000+). All endpoints require **ADMIN** auth — archives may contain world data, plugin configs, and database dumps.
...
| Parameter | Description |
|---|---|
| `target` | Filter by exact target name (service / group / `all` for aggregate scopes) |
  ...
  "backups": [
    {
      ...
      "scheduleName": "hourly",
      "startedAt": "2026-04-15T08:30:00Z",
      "completedAt": "2026-04-15T08:30:12Z",
      "status": "SUCCESS",
      "sizeBytes": 58720256,
      "archivePath": "service-Lobby-1-hourly-20260415-083000.tar.zst",
      "checksum": "a1b2c3…",
      "errorMessage": null,
      "nodeId": "local",
...
Run a backup now. Body is optional — with an empty `targets` list, all scopes enabled in config are backed up.
...
`targets` accepts `services`, `dedicated`, `templates`, `config`, `database`, `state_sync`, or omit entirely.
`scheduleClass` defaults to `manual` — manual backups are immune to automatic retention pruning when `retention.keep_manual = true`.
`target` (optional) narrows SERVICE/DEDICATED backups to one specific name.

Response is the same `BackupListResponse` shape as `GET /api/backups`, containing the rows that were just produced (one row per archive).

### DELETE /api/backups/{'{'}id{'}'}

Delete the backup record and its archive file from disk.
...
`targetPath` (optional): override the destination directory. Defaults to the original location (`registry.get(name).workingDirectory` for services, etc.).
`dryRun`: list files that would be extracted without writing.
`force`: overwrite even when the target service is `READY` / `STARTING` / `DRAINING`. Without `force`, restore refuses on a running service.
...
```json title="Response (200 OK)"
{
  ...
  "dryRun": false,
  "filesExtracted": 1234,
  "files": ["world/region/r.0.0.mca", "..."]
}
```

### POST /api/backups/{'{'}id{'}'}/verify

Re-read the archive and recompute SHA-256 for every entry against the trailing `MANIFEST.sha256`.
...
Stream the raw `.tar.zst` archive. `Content-Disposition: attachment` set. Bearer auth required — a plain `<a download>` won't work; fetch + blob download on the client side.

### GET /api/backups/{'{'}id{'}'}/manifest

Stream the `MANIFEST.sha256` entry from inside the archive as `text/plain` — one `<hex-sha256>  <relative/path>` line per file. Useful for ad-hoc integrity checks.

### GET /api/backups/schedules

List configured schedules with computed next-fire times and last-run status.
...
[
  {
    ...
    "lastRunAt": "2026-04-15T08:00:00Z",
...
Scheduler summary for the Overview tab — active-jobs count, local destination, schedules.
...
Both fields optional. `retentionClass` narrows the prune pass to one class; omit to prune all classes.
...
Full serialized module config — `BackupModuleConfig` shape:

```json title="Response (200 OK)"
{
  "enabled": true,
  "localDestination": "data/backups",
  "maxConcurrent": 2,
  "compressionLevel": 3,
  "compressionWorkers": 0,
  "quiesceServices": true,
  "quiesceWaitSeconds": 2,
  "scope": {
    "services": true,
    "dedicated": true,
    "templates": true,
  ...
  "excludes": ["logs/**", "*.log", "cache/**", "*.lock"],
  "schedules": [
    { "name": "hourly", "cron": "0 * * * *", "retentionClass": "hourly", "targets": ["services", "dedicated", "database"] }
  ],
  "retention": {
    "hourlyKeep": 24,
    "dailyKeep": 7,
...
Replace the module config. Body is the same shape as the `GET` response. Validates server-side (cron syntax, compression level 1..22, unique schedule names matching `[A-Za-z0-9_-]+`, allowed target keys, known retention classes) and atomically rewrites `config/modules/backup/backup.toml` on success. The scheduler is hot-reloaded. Returns the persisted config.

`400 VALIDATION_FAILED` on any schema error — the in-memory config is not touched.
