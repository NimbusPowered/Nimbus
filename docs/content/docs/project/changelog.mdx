...
- **API-Stability-Policy** — neue Referenzseite `docs/reference/api-stability.mdx` dokumentiert die SemVer-Garantien für den `/api/*`-URL-Space (additive Minor-Changes, Removals/Breaks nur in Major-Versionen), den `Deprecation:`-Header-Pfad inkl. Sunset-Fenster, den Status der Error-Codes (`ApiError`-Enum) als Teil des Wire-Kontrakts und die Abgrenzung zum eigenständig versionierten Cluster-Protokoll (`protocolVersion`). Begründet zusätzlich, warum Nimbus bewusst **kein** `/api/v1/`-URL-Prefix einführt (private Admin-API, Consumer werden gemeinsam mit dem Controller ausgeliefert)
- **`NimbusEvent.actor`-Grammatik** — das `actor: String`-Feld auf `NimbusEvent` hat jetzt eine explizite KDoc-Grammatik: entweder ein bloßer Typ oder `<type>:<id>`, mit `type ∈ { "system", "console", "api", "player", "agent" }`. Neue Extension `NimbusEvent.actorType()` liefert den Typ-Präfix (alles vor dem ersten `:`) für Filtering und Auditing. Damit ist das bisher konventions-getragene Format von Audit-Log- und Event-Stream-Einträgen vertraglich festgehalten
